...
' console window on login. This VBS runs it with window style 7
' (minimized, no focus) — ntfy needs a console to stay alive, so
' style 0 (hidden) causes it to exit immediately. Registered in
' HKCU\...\Run by setup.ps1.
'
...
sh.Run """" & home & "\scoop\apps\ntfy\current\ntfy.exe"" subscribe --from-config", 7, False